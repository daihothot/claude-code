...
| **上下文** | 仅任务描述 | 父 Agent 的对话历史（会过滤未完成的 `tool_use`） |
...
- **命名 Agent**：基础工具池会独立组装（`AgentTool.tsx:573-577`）
- **Fork 子进程**：直接继承父 Agent 的精确工具数组（`useExactTools: true`），以保持 Prompt Cache 前缀字节一致
...
关键设计决策：
- **命名 Agent 尝试覆盖权限模式**：默认使用 `selectedAgent.permissionMode`（默认 `acceptEdits`），但父 Agent 处于 `bypassPermissions`、`acceptEdits`，以及特性开启时的 `auto` 模式时，父级模式优先
- **MCP 工具继承**：`appState.mcp.tools` 包含所有已连接的 MCP 工具，命名 Agent 在独立组装时自动获得
...
`resolveAgentTools()` 会根据 Agent 定义中的 `tools` 字段过滤可用工具，将 `['*']` 映射为全量工具。注意：Fork 路径不会走这层过滤，而是直接使用父 Agent 的 `availableTools`。
...
当 Agent 进入异步路径时，会立即返回 `async_launched` 状态（`AgentTool.tsx:686-764`）。常见触发条件包括：

- `run_in_background=true`
- `selectedAgent.background=true`
- coordinator 模式
- fork-subagent 实验开启
- KAIROS / assistant 模式强制异步
- proactive 模式激活
...
对于一次性内置 Agent（Explore、Plan），在**没有 worktree 信息附加**时会省略 `<usage>` 块——每周节省约 1-2 Gtok 的上下文窗口；如果结果里附带 `worktreePath/worktreeBranch`，尾部 trailer 仍会保留。
